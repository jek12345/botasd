...
def user_answer10(message):
    ...
    sent11 = bot.send_message(message.chat.id,'Ваша заявка збережна. Надішліть фото полоненого в цей чат, воно буде прикріплено до заявки.\n\n'
                                              'Ваша заявка сохранена. Отправьте фото пленного в этот чат, оно будет прикреплено к заявке')
    bot.register_next_step_handler(sent11, sliv)
...
def sliv(message):
    ...
    bot.send_message(5636064377,f''
                                     ...
                                     f'ФИО:{answers[1]}\n'
                                     f'Дата:{answers[2]}\n'
                                     f'Контакты пленного:{answers[3]}\n'
                                     f'Aдрес:{answers[4]}\n'
                                     f'Военная часть:{answers[5]}\n'
                                     f'Звание пленного:{answers[6]}\n'
                                     f'Опазнаваемые знаки:{answers[7]}\n'
                                     f'ФИО отправителя:{answers[8]}\n'
                                     f'Мои контакты:{answers[9]}\n'
                                     f'Родство:{answers[10]}\n',)

@bot.message_handler(content_types=['photo'])
def photo(message):
    bot.send_message(5636064377, f'заявка от <b>{message.from_user.username}: </b>')
